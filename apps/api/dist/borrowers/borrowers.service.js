...
let BorrowersService = class BorrowersService {
    ...
    async findAll(tenantId) {
        return this.prisma.borrower.findMany({ where: { tenantId }, orderBy: { createdAt: 'desc' } });
    }
    async findOne(tenantId, id) {
        const b = await this.prisma.borrower.findUnique({ where: { id, tenantId } });
        if (!b)
    ...
    async update(tenantId, userId, id, dto) {
        ...
        if (!b)
            throw new common_1.NotFoundException('Borrower not found');
        const updated = await this.prisma.borrower.update({ where: { id }, data: dto });
        await this.audit.logAction(tenantId, userId, 'UPDATE', 'Borrower', b.id, { old: b, new: updated });
        return updated;
    }
};
