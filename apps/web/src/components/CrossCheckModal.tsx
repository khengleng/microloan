...
interface CreditCheckResult {
    scope: string;
    found: boolean;
    loans: { status: string; date: string }[];
    bureau: { available: boolean; message: string };
}
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Check this borrower&apos;s prior activity within your organization. Cross-lender obligations are checked through the Credit Bureau (CBC).
                    </DialogDescription>
                </DialogHeader>
                ...
                {results && (
                    <div className="mt-8 space-y-4 animate-in fade-in slide-in-from-top-4">
                        <h4 className="text-sm font-bold uppercase tracking-wider text-slate-500">Results — Your Organization</h4>
                        {!results.found ? (
                            <div className="p-6 text-center border-2 border-dashed rounded-xl bg-green-50/30 border-green-100">
                                <CheckCircle2 className="mx-auto text-green-500 mb-2" size={32} />
                                <p className="font-semibold text-green-700">No prior activity in your organization.</p>
                                <p className="text-xs text-green-600">This borrower has no recorded loans with you.</p>
                            </div>
                        ) : (
                            <div className="border rounded-xl p-4 bg-white shadow-sm border-slate-100">
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <div className="text-xs text-slate-400">{results.loans.length} Loan Records</div>
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    {results.loans.map((loan, li) => (
                        ...
                        <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-lg border border-amber-100 text-[11px] text-amber-700">
                            <AlertCircle size={14} className="mt-0.5 shrink-0" />
                            <span>{results.bureau.message}</span>
                        </div>
                    </div>
                )}
            </DialogContent>
