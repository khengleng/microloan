...
function bureauPoints(grade) {
    const g = (grade || '').toUpperCase();
    if (['A', 'EXCELLENT', 'AA'].includes(g))
        return 30;
    if (['B', 'GOOD'].includes(g))
        return 24;
    if (['C', 'FAIR'].includes(g))
        return 15;
    if (['D', 'POOR'].includes(g))
        return 8;
    if (['E', 'BAD'].includes(g))
        return 3;
...
export function computeScorecard(input) {
    ...
    const disposable = input.monthlyIncome != null
    ...
    let decision = score >= 65 ? 'APPROVE' : score >= 45 ? 'REFER' : 'DECLINE';
    ...
    if (dsr != null && dsr > 0.6) {
        decision = 'DECLINE';
        reasons.push('Debt-service ratio above 60%');
    }
