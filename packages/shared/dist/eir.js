 ...
 */
export function monthlyIrr(netDisbursed, installments) {
    ...
    const npv = (r) => installments.reduce((acc, amt, i) => acc + amt / Math.pow(1 + r, i + 1), 0) - netDisbursed;
    let lo = 0;
    let hi = 1; // 100% per month upper bound; expand if needed
    while (npv(hi) > 0 && hi < 100)
        hi *= 2;
    for (let i = 0; i < 200; i++) {
        const mid = (lo + hi) / 2;
        const v = npv(mid);
